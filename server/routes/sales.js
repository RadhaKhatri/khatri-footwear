...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // Verify stock and collect product info
    const lineItems = []
    for (const item of items) {
      const { rows } = await client.query('SELECT * FROM products WHERE id=$1 FOR UPDATE', [item.product_id])
      if (!rows.length) throw new Error(`Product ID ${item.product_id} not found.`)
      const product = rows[0]
      if (product.quantity < item.quantity) {
        throw new Error(`Insufficient stock for ${product.brand} ${product.article_number}. Available: ${product.quantity}`)
      }
      lineItems.push({ product, quantity: item.quantity, unit_price: parseFloat(item.unit_price) || parseFloat(product.selling_price) })
    }
    ...
    const { rows: saleRows } = await client.query(
      `INSERT INTO sales
        (invoice_number, customer_name, customer_phone, subtotal_amount, discount_amount, tax_amount, total_amount, payment_mode, sale_date)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,CURRENT_DATE)
       RETURNING *`,
      [invoiceNumber, customer_name, customer_phone, subtotal.toFixed(2), discAmt.toFixed(2), taxAmt.toFixed(2), total.toFixed(2), payment_mode]
    )
    ...
    for (const item of lineItems) {
      const { rows: itemRows } = await client.query(
        `INSERT INTO sale_items
          (sale_id, product_id, brand, article_number, size, color, quantity, unit_price, total_price)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
         RETURNING *`,
        [sale.id, item.product.id, item.product.brand, item.product.article_number,
         item.product.size, item.product.color, item.quantity,
         item.unit_price.toFixed(2), (item.unit_price * item.quantity).toFixed(2)]
      )
...
// GET /api/sales — list sales with period filter
...
// GET /api/sales/invoice/:number — get single invoice with items
router.get('/invoice/:number', async (req, res) => {
  try {
    const { rows: saleRows } = await pool.query('SELECT * FROM sales WHERE invoice_number=$1', [req.params.number])
    if (!saleRows.length) return res.status(404).json({ error: 'Invoice not found.' })
    const { rows: items } = await pool.query('SELECT * FROM sale_items WHERE sale_id=$1', [saleRows[0].id])
