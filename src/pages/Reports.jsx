...
import { Download, Lock, Eye, EyeOff, TrendingUp, TrendingDown, ShieldCheck } from 'lucide-react'
...
export default function Reports() {
  ...
  // Check if a report password exists
  useEffect(() => {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => setUnlocked(false)} className="btn-secondary btn-sm flex items-center gap-1.5">
            <Lock size={13} /> Lock
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
                ...
                <div key={label} className={`rounded-xl p-4 border ${isProfit ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                  ...
                  <div className="space-y-1 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">₹{revenue.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Purchase Cost</span>
                      <span className="font-semibold text-red-500">₹{cost.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="flex justify-between pt-1 border-t border-gray-200">
                      <span className="text-gray-500">Margin</span>
                      <span className={`font-bold ${isProfit ? 'text-green-700' : 'text-red-600'}`}>{margin}%</span>
                    </div>
                  </div>
                </div>
              )
                    ...
                    {pl.products.map((p, i) => {
                      ...
                      return (
                        <tr key={i} className={`border-t border-gray-50 hover:bg-gray-50 ${p.profit < 0 ? 'bg-red-50/30' : ''}`}>
                          ...
                          <td className="px-3 py-2.5 text-right">₹{p.revenue.toLocaleString('en-IN')}</td>
                          <td className="px-3 py-2.5 text-right text-red-500">₹{p.cost.toLocaleString('en-IN')}</td>
                          ...
                          <td className="px-3 py-2.5 text-center">
                            <span className={margin >= 30 ? 'badge-green' : margin >= 15 ? 'badge-amber' : 'badge-red'}>
                              {margin}%
                            </span>
                          </td>
                          <td className="px-3 py-2.5 text-center">
          ...
          {/* ── SECTION 3: Inventory Value (sensitive) ── */}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            {[
              ...
              ['Inventory Value',   `₹${(data?.inventoryValue ?? 0).toLocaleString('en-IN')}`,                  'purchase cost'],
                ...
                {monthly.map(m => {
                  ...
                  return (
                    <div key={m.month} className="flex flex-col items-center gap-0.5 flex-1 min-w-0 h-full justify-end">
                      <div className="text-xs font-semibold text-blue-900 truncate" style={{ fontSize: '9px' }}>
                        {m.revenue > 0 ? `₹${Math.round(m.revenue / 1000)}k` : ''}
                      </div>
                      <div className="w-full relative flex flex-col justify-end" style={{ height: '80px' }}>
                        <div className="w-full bg-blue-200 rounded-t-sm" style={{ height: `${Math.max(revPct * 0.8, 2)}px` }} title={`Revenue ₹${m.revenue.toLocaleString('en-IN')}`} />
                        <div className="absolute bottom-0 w-full bg-green-500 rounded-t-sm opacity-70" style={{ height: `${Math.max(profPct * 0.8, 0)}px` }} title={`Profit ₹${m.profit.toLocaleString('en-IN')}`} />
                      </div>
                      ...
                      <tbody>
                        {topProducts.map((p, i) => (
                          <tr key={i} className="border-t border-gray-50 hover:bg-gray-50">
                            ...
                            <td className="px-3 py-2.5 text-center">{p.size}</td>
                            <td className="px-3 py-2.5 text-center font-bold">{p.units_sold}</td>
                            <td className="px-3 py-2.5 text-right font-bold text-blue-900">₹{Number(p.revenue).toLocaleString('en-IN')}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    ...
                    <tbody>
                      ...
                      {sales.map(s => (
                        <tr key={s.id} className="hover:bg-gray-50 border-t border-gray-50">
                          ...
                          <td className="table-cell font-bold text-blue-900">₹{Number(s.total_amount).toLocaleString('en-IN')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="card p-5">
                ...
                {!data?.paymentBreakdown?.length ? (
                  ...
                  <div key={p.payment_mode} className="flex justify-between items-center mb-3 last:mb-0 pb-3 border-b border-gray-50 last:border-0">
                    ...
                    <div className="font-bold text-blue-900 text-sm">₹{Number(p.revenue).toLocaleString('en-IN')}</div>
                  </div>
                ))}
              </div>
